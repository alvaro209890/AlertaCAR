/**
 * Shared geo-processing utilities (portado do GeoForest-IA, backend/geo-utils.ts).
 * Usado pelas ferramentas de geometria da Fase 13 (validate-geometry, vertices-proximas,
 * containment, processar-geo) em backend/src/routes/tools.ts.
 */
...
/**
 * Teto de bytes descomprimidos somados de todo o ZIP — proteção contra "zip bomb"
 * (um .zip pequeno criado propositalmente pra inflar em memória até esgotar o servidor).
 * Shapefiles legítimos de um imóvel ficam muito abaixo disso; 300 MB é generoso mas finito.
 * Adicionado no port pra Fase 13 do AlertaCAR — não existia no GeoForest original porque lá
 * o upload já passava por outras camadas de limite (job assíncrono com fila).
 */
const MAX_TOTAL_INFLATED_BYTES = 300 * 1024 * 1024;
...
export function extractZipEntries(zipBuffer: Buffer) {
    ...
    for (let i = 0; i < totalEntries; i += 1) {
        ...
        if (totalInflated > MAX_TOTAL_INFLATED_BYTES) {
            throw new Error("ZIP excede o limite de conteúdo descomprimido (300 MB) — possível arquivo corrompido ou malicioso.");
        }
