/**
 * Portado de GeoForest-IA/backend/vertices-proximas.ts (Fase 13.2 do AlertaCAR).
 * Removido: registro de rotas Express, fila de job assíncrono (processing-jobs)
 * e armazenamento de arquivo do usuário (local-storage) — infraestrutura que
 * não existe no AlertaCAR; o caso de uso aqui é síncrono (ver routes/tools.ts).
 * A lógica de geometria/CRS abaixo é idêntica ao original.
 */
import archiver from "archiver";
import crypto from "node:crypto";
import path from "node:path";
import proj4 from "proj4";
import { extractZipEntries, detectUtmProj } from "./geo-utils.js";
import {
  buildDbfBuffer,
  buildPointShpAndShx,
  type DbfFieldDef,
